...
4. Place `vault_render_optimization.0.3.3.jar` in the directory.
...
Stop Minecraft before replacing the jar. VRO 0.3.3 requires no cache deletion
...
VRO does not alter world saves or server data. The configuration file is the
only persistent VRO state.
